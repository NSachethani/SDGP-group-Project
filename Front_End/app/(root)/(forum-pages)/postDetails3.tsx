import {
  ...
  import PostCard from "@/components/PostCard3";
  import Input from "@/components/Input";
  import Icon from "@/constants/icon";
  import CommentItem from "@/components/CommentItem";
  import { createNotification3 } from "@/service/notificationService";
  ...
  const postDetails = () => {
    ...
    useEffect(() => {
      const backAction = () => {
        router.replace("/(root)/(forum-pages)/Digital-Wellness");
        return true;
      };
    ...
    const handleNewComment = async (payload: any) => {
      ...
      if (payload.new) {
        ...
        let res = await getUserData3(newComment.user_id);
        newComment.user = res.success ? res.data : {};
        setPost((prevPost: any) => {
          return {
            ...prevPost,
            DW_PostComments: [newComment, ...prevPost.DW_PostComments],
          };
        });
      }
    };
    ...
    useEffect(() => {
      let commentChannel = supabase
        .channel("DW_PostComments")
        .on(
          "postgres_changes",
          {
            event: "INSERT",
            schema: "public",
            table: "DW_PostComments",
    ...
    const getPostDetails = async () => {
      let res = await fetchPostDetails3(postId as any);
    ...
    const reloadScreen = () => {
      router.replace({
        pathname: "/(root)/(forum-pages)/postDetails3",
        params: { postId },
      });
    };
    ...
    const onNewComment = async () => {
      ...
      setLoading(true);
      let res = await createPostComment3(data);
      setLoading(false);
      if (res.success) {
    ...
    const onDeleteComment = async (comment: any) => {
      ...
      let res = await removeComment3(comment.id);
      if (res.success) {
        setPost((prevPost: any) => {
          let updatedPost = { ...prevPost };
          updatedPost.DW_PostComments = updatedPost.DW_PostComments.filter(
    ...
    const onDeletePost = async (item: any) => {
      ...
      let res = await removePost3(item.id);
      if (res.success) {
        router.replace("/(root)/(forum-pages)/Digital-Wellness");
      } else {
      ...
      >
        <ScrollView className="px-4">
          ...
          <View className="flex flex-col gap-2 mt-4">
            ...
            {post?.DW_PostComments?.map((comment: any) => (
              <CommentItem
