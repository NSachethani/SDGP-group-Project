...
import {
  createOrUpadatePost5,
  fetchPosts5,
  getUserData5,
  removePost5,
} from "@/service/postService";
import PostCard4 from "@/components/PostCard5";
...
const TechnicalHelp = () => {
  ...
  const handlePostEvent = async (payload: any) => {
    if (payload.eventType === "INSERT" && payload?.new?.id) {
      ...
      let res = await getUserData5(newPost.user_id);
      newPost.postLikes=[];
  ...
  const getPosts = async () => {
    ...
    let res = await fetchPosts5(limit);
  ...
  useEffect(() => {
    let postChannel = supabase
      ...
      .on(
        "postgres_changes",
        { event: "*", schema: "public", table: "TH_Posts" },
        handlePostEvent
      )
      .subscribe();
    getPosts();
  ...
  const onSubmit = async () => {
    ...
    setLoading(true);
    let res = await createOrUpadatePost5(data);
    setLoading(false);
