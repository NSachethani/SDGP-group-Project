import {
  ...
  import { createNotification, createNotification2 } from "@/service/notificationService";
  ...
  const postDetails = () => {
    ...
    const handleNewComment = async (payload: any) => {
      ...
      if (payload.new) {
        ...
        let res = await getUserData2(newComment.user_id);
        newComment.user = res.success ? res.data : {};
        setPost((prevPost: any) => {
          return {
            ...prevPost,
            AH_PostComments: [newComment, ...prevPost.AH_PostComments],
          };
        });
      }
    };
    ...
    useEffect(() => {
      let commentChannel = supabase
        .channel("AH_PostComments")
        .on(
          "postgres_changes",
          {
            event: "INSERT",
            schema: "public",
            table: "AH_PostComments",
    ...
    const getPostDetails = async () => {
      let res = await fetchPostDetails2(postId as any);
    ...
    const onNewComment = async () => {
      ...
      setLoading(true);
      let res = await createPostComment2(data);
      setLoading(false);
      if (res.success) {
    ...
    const onDeleteComment = async (comment: any) => {
      ...
      let res = await removeComment2(comment.id);
      if (res.success) {
        setPost((prevPost: any) => {
          let updatedPost = { ...prevPost };
          updatedPost.AH_PostComments = updatedPost.AH_PostComments.filter(
      ...
      >
        <ScrollView className="px-4">
          ...
          <View className="flex flex-col gap-2 mt-4">
            ...
            {post?.AH_PostComments?.map((comment: any) => (
              <CommentItem
