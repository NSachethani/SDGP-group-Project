import {
  ...
  import PostCard from "@/components/PostCard5";
  import Input from "@/components/Input";
  import Icon from "@/constants/icon";
  import CommentItem from "@/components/CommentItem";
  import { createNotification5 } from "@/service/notificationService";
  ...
  const postDetails = () => {
    ...
    useEffect(() => {
      const backAction = () => {
        router.replace("/(root)/(forum-pages)/Technical-Help");
        return true;
      };
    ...
    const handleNewComment = async (payload: any) => {
      ...
      if (payload.new) {
        ...
        let res = await getUserData5(newComment.user_id);
        newComment.user = res.success ? res.data : {};
        setPost((prevPost: any) => {
          return {
            ...prevPost,
            TH_PostComments: [newComment, ...prevPost.TH_PostComments],
          };
        });
      }
    };
    ...
    useEffect(() => {
      let commentChannel = supabase
        .channel("TH_PostComments")
        .on(
          "postgres_changes",
          {
            event: "INSERT",
            schema: "public",
            table: "TH_PostComments",
    ...
    const getPostDetails = async () => {
      let res = await fetchPostDetails5(postId as any);
    ...
    const onNewComment = async () => {
      ...
      setLoading(true);
      let res = await createPostComment5(data);
      setLoading(false);
      if (res.success) {
    ...
    const onDeleteComment = async (comment: any) => {
      ...
      let res = await removeComment5(comment.id);
      if (res.success) {
        setPost((prevPost: any) => {
          let updatedPost = { ...prevPost };
          updatedPost.TH_PostComments = updatedPost.TH_PostComments.filter(
    ...
    const onDeletePost = async (item: any) => {
      ...
      let res = await removePost5(item.id);
      if (res.success) {
        router.replace("/(root)/(forum-pages)/Technical-Help");
      } else {
      ...
      >
        <ScrollView className="px-4">
          ...
          <View className="flex flex-col gap-2 mt-4">
            ...
            {post?.TH_PostComments?.map((comment: any) => (
              <CommentItem
