...
const DigitalWellness = () => {
  ...
  const handlePostEvent = async (payload: any) => {
    if (payload.eventType === "INSERT" && payload?.new?.id) {
      ...
      let res = await getUserData3(newPost.user_id);
      newPost.postLikes=[];
  ...
  const getPosts = async () => {
    ...
    let res = await fetchPosts3(limit);
  ...
  useEffect(() => {
    let postChannel = supabase
      ...
      .on(
        "postgres_changes",
        { event: "*", schema: "public", table: "DW_Posts" },
        handlePostEvent
      )
      .subscribe();
    getPosts();
  ...
  const onSubmit = async () => {
    ...
    setLoading(true);
    let res = await createOrUpadatePost3(data);
    setLoading(false);
