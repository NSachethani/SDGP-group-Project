...
const OfflineBliss = () => {
  ...
  const handlePostEvent = async (payload: any) => {
    if (payload.eventType === "INSERT" && payload?.new?.id) {
      ...
      let res = await getUserData4(newPost.user_id);
      newPost.postLikes=[];
  ...
  const getPosts = async () => {
    ...
    let res = await fetchPosts4(limit);
  ...
  useEffect(() => {
    let postChannel = supabase
      ...
      .on(
        "postgres_changes",
        { event: "*", schema: "public", table: "OB_Posts" },
        handlePostEvent
      )
      .subscribe();
    getPosts();
  ...
  const onSubmit = async () => {
    ...
    setLoading(true);
    let res = await createOrUpadatePost4(data);
    setLoading(false);
