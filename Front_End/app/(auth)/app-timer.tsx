...
const Timer = () => {
  ...
  // // Periodically check usage for each app and send notification if exceeded threshold
  // useEffect(() => {
  //   const intervalId = setInterval(() => {
  //     ScreenTimeModule.getUsageData()
  //       .then((data: any) => {
  //         if (data.days && data.days.length > 0 && data.days[0].apps) {
  //           const todayApps = data.days[0].apps;
  //           todayApps.forEach((app: any) => {
  //             const appName = app.name.toLowerCase();
  //             if (reminderTimes[appName]) {
  //               const thresholdMs = reminderTimes[appName] * 60000;
  //               console.log(
  //                 `App ${appName}: screenTime=${app.screenTime}, threshold=${thresholdMs}`
  //               ); // debug log
  //               if (app.screenTime > thresholdMs) {
  //                 // Schedule immediate notification with a small delay
  //                 Notifications.scheduleNotificationAsync({
  //                   content: {
  //                     title: `Screen time of ${appName} is exceeded!`,
  //                     body: `You have reached the maximum screen time of ${appName}. Take a break!`,
  //                   },
  //                   trigger: {
  //                     seconds: 1,
  //                     repeats: false,
  ...
  // Save reminder for a selected app
                      ...
                      renderItem={({ item }) => (
                        <Text
                          style={{
                            ...
                            textAlign: "center",
                            fontSize: 16,
                            color: item === selectedHours ? "#8D5395" : "#000",
                          }}
                        >
                          {item}
                        </Text>
