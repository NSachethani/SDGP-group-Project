...
import {createPostLike4, RemovePostLike4 } from "@/service/postService";
...
const PostCard = ({
  ...
  const onLike = async () => {
    if (liked) {
      ...
      let res = await RemovePostLike4(item?.id, currentUser?.id);
      if (res.success) {
      ...
      setLikes([...likes, data]);
      let res = await createPostLike4(data);
      if (res.success) {
        console.log("Liked");
      }
      if (!res.success) {
  ...
  console.log("post item comments:", item?.OB_PostComments);
  return (
    <View
