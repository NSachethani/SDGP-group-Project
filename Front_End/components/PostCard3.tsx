...
import { createPostLike3, RemovePostLike3 } from "@/service/postService";
...
const PostCard = ({
  ...
  const onLike = async () => {
    if (liked) {
      ...
      let res = await RemovePostLike3(item?.id, currentUser?.id);
      if (res.success) {
      ...
      setLikes([...likes, data]);
      let res = await createPostLike3(data);
      if (res.success) {
        console.log("Liked");
      }
      if (!res.success) {
  ...
  console.log("post item comments:", item?.DW_PostComments);
  return (
    <View
