...
import {createPostLike5, RemovePostLike5 } from "@/service/postService";
...
const PostCard = ({
  ...
  const onLike = async () => {
    if (liked) {
      ...
      let res = await RemovePostLike5(item?.id, currentUser?.id);
      if (res.success) {
      ...
      setLikes([...likes, data]);
      let res = await createPostLike5(data);
      if (res.success) {
        console.log("Liked");
      }
      if (!res.success) {
  ...
  console.log("post item comments:", item?.TH_PostComments);
  return (
    <View
